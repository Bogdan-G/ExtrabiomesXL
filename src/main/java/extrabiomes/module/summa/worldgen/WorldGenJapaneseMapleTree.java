...
public class WorldGenJapaneseMapleTree extends WorldGenNewTreeBase
{
    ...
    public void generateVerticalCone(World world, int x, int y, int z, int height, double r1, double r2, ItemStack leaves)
    {
        ...
        {
            placeLeavesCircle(x, y + offset, z, (float)((ratio * offset) + r1), leaves, world);
        }
    }
    
    public boolean checkCanopyLayer(World world, int x, int y, int z, double radius)
    {
        final float minDist = (radius - 2 > 0) ? (float)((radius - 2) * (radius - 2)) : -1;
        final int maxDist = (int)(radius * radius);
